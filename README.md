...
Convolutional neural networks (CNNs) are a method of machine learning used for image classification and visual tasks. CNNs are based on the visual cortex of the brain and several recent advancements have increased the use of CNNs in many upcoming projects. CNNs consist of several convolutional layers which an image is passed through to get an output. Convolution layers extract features from the image. Training data must be used for CNNs to adjust the weights and biases which contribute to the final accuracy. A convolutional neural network can be used for this project because the goal of the program is to classify road signs which is a visual task.

CNNs can be implemented into code using several methods, however one of the most common is Keras. Keras was developed by Google and is a deep learning API that can be used to facilitate the use of CNNs. Keras was written in python and can currently only be used in python code.

## Dataset
The dataset used for this code was taken from Kaggle: https://www.kaggle.com/datasets/andrewmvd/road-sign-detection. The name of the dataset is “Road Sign Detection” and was created by the user, LARXEL. The dataset consists of 877 images of road signs which are categorized in four classes: crosswalk signs, traffic lights, speed limit signs, and stop signs. The dataset also has bounding box information, which labels where the road sign is located in the image. For the purposes of our project, we ignore the bounding boxes.
## Classification
